...
-- En 'Be%' el % índica un número indeterminado de caracteres 
...
SELECT language, region FROM world.countrylanguage
INNER JOIN world.country ON country.code = countrylanguage.countrycode
WHERE region = 'caribbean';
-- Distinct(nombre-de-columna) es un método para que los resultados no se repitan
...
SELECT name, lifeExpentacy FROM world.country
...
-- Ciudades de sur america con poblacion mayor a medio millón
SELECT city.name, continent FROM world.country
	JOIN world.city ON country.code = city.countrycode
